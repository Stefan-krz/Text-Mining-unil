...
lyrics.lsa <- textmodel_lsa(lyrics.dfm, nd=5)
...
#run again but this time filtering words like topic modelling
lyrics.tokens <- tokens_tolower(lyrics.tokens) %>% tokens_wordstem() %>%
...
lyrics.lsa <- textmodel_lsa(lyrics.tfidf, nd=10)
...
df <- data.frame(Class=y, X=lyrics.lsa$docs)
df <- cbind(df, song_value = final_lyrics$value, TTR =final_lyrics$TTR)
df.tr <- df[index.tr,]
df.te <- df[-index.tr,]
lyrics.fit <- ranger(Class ~ ., 
                     data = df.tr, 
